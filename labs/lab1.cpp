...
const int NUM_STUDENTS = 25; // max number of scores
...
int main() {
  ...
  while (inputValue >= 0 && numScores < NUM_STUDENTS) {
    ...
    cout << "Enter a score (-1 when done): "; // input
    cin >> inputValue; // keyboard input
  }
...
float average(int data[], int dataLen) {
  ...
  float currentAverage = static_cast<float>(totalVal) / static_cast<float>(dataLen);
